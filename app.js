...
request(apiUrl, function(error, response, body) {
  ...
  /*
  fs.writeFile('./resource/sitemap.html', sitemapHtml, function(err) {
    if (err) console.log('ERROR Saving!');
    else console.log('UPDATED FILE: ./resource/sitemap.html');
  });
  */
});
...
function fCreateCatArr(sitemapHtml) {
  ...
  for (var i = 0; i < firstLvlQuantity; i++) {
  ...
  for (var i = 0; i < firstLvlCatsArr.length; i++) {
    ...
    if (childrenLength > 0) {
      ...
      for (var k = 0; k < childrenLength; k++) {
        ...
        if (trdLvlChldrLen) {
          ...
          for (var l = 0; l < childrenLength3; l++) {
